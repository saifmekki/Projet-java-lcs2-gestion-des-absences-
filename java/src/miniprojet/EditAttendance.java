...
public class EditAttendance {
	...
	public void editView() throws SQLException {
		...
		del.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				String adding = "DELETE FROM Absence WHERE class = '" + String.valueOf(clss.getSelectedItem())
	...
	public ResultSet dbSearch(String classes, String dt) throws SQLException {
		String str1 = "SELECT * from Absence, Etudiant where Absence.id_absence= Etudiant.id AND Absence.class = '"
				+ classes + "' AND Absence.dt = '" + dt + "'";
		Statement stm = con.createStatement();
	...
	public void tblupdt(String classes, String dt) {
		try {
			...
			for (int i = 0; res.next(); i++) {
				...
				model.setValueAt(res.getInt("id_absence"), i, 0);
	...
	public void editItem(int id, String status, String date) throws SQLException {
		String adding = "UPDATE Absence SET status = '" + status + "' WHERE id_absence = " + id + " AND dt = '" + date
				+ "'";
		Statement stm = con.createStatement();
		stm.executeUpdate(adding);
	}
